...
from bottle import run, view, abort
...
class Entry:
    ...
    #     if self.get('annotation'):
    #         pages = json.loads(self.get('annotation'))
    #         for page, marks in pages.iteritems():
    #             self.project.addmarkings(db, self.id, self.user, page, marks)
...
class Project:
    ...
    def contribute(self, db, uid, entry):
        ...
        query = """INSERT INTO
                   ...
                   VALUES(?, ?, ?, ?, ?, ?, ?)"""
        db.execute(query, [id, uid, self.slug, now,
                json.dumps(dict(entry)), finished, now])

    def addmarkings(self, db, entry, uid, page, data):
        now = str(datetime.datetime.now())
        query = """INSERT INTO
                   markings(id, user, project, date, post, page, markings)
                   VALUES(?, ?, ?, ?, ?, ?, ?)"""
        db.execute(query, [str(uuid.uuid4()), uid, self.slug, now, entry,
...
@get('/project/<slug>/touch')
def touch(slug, db):
    project = Project.find(slug)
    response.content_type = "application/json; charset=utf-8"
    for entry in Entry.all(db):
        entry.touch(db)
